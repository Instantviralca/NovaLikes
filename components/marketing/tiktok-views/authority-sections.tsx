...
import {
  TIKTOK_VIEWS_PAGE_CONFIG,
  type TikTokViewsPageConfig,
} from '@/data/content/tiktok-views-page-config';
import { cn } from '@/lib/utils';
import type { InternalLink } from '@/types/linking';

type SectionProps = {
  config?: TikTokViewsPageConfig;
  className?: string;
};

/** Direct-answer block — Can you buy TikTok views? */
export function TikTokViewsCanYouBuy({
  ...
  return (
    ...
    >
      <Container size="lg">
        <FadeUp className="mx-auto max-w-3xl space-y-3">
          <Heading
            as="h2"
            size="h2"
...
/** How views support visibility — pairs with Why Do People Buy */
export function TikTokViewsSupportVisibility({
  config = TIKTOK_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
  const { supportVisibility } = config;

  return (
    <Section
      id={supportVisibility.id}
      spacing="lg"
      className={cn('bg-white', className)}
      aria-labelledby={`${supportVisibility.id}-heading`}
    >
      <Container size="lg">
        <FadeUp className="mx-auto max-w-3xl space-y-3">
          <Heading
            as="h2"
            size="h2"
            id={`${supportVisibility.id}-heading`}
            className="tracking-tight"
          >
            {supportVisibility.title}
          </Heading>
          <MutedText className="text-base leading-relaxed sm:text-[1.05rem]">
            {supportVisibility.description}
          </MutedText>
        </FadeUp>
      </Container>
    </Section>
...
/** Does buying views work — help / limits + analytics visual */
export function TikTokViewsDoesBuyingWork({
  config = TIKTOK_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
      ...
      <Container size="xl">
        ...
        <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.1fr)_minmax(0,0.95fr)] lg:gap-5 xl:gap-7">
          ...
          <FadeUp delay={0.04} className="flex justify-center lg:px-1">
            <TikTokViewsAnalyticsDashboard className="w-full max-w-[26rem]" />
          </FadeUp>
...
export function TikTokViewsWhatHappens({
  config = TIKTOK_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
/** Compact worth-it direct answer */
export function TikTokViewsWorthIt({
  config = TIKTOK_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
  const { worthIt } = config;
  ...
  return (
    <Section
      ...
      spacing="lg"
      className={cn('bg-white', className)}
      aria-labelledby={`${worthIt.id}-heading`}
    >
      <Container size="lg">
        <FadeUp className="mx-auto max-w-3xl space-y-3">
          <Heading
...
/** Should you buy — PAA / AI Overview direct answer */
export function TikTokViewsShouldYouBuy({
  config = TIKTOK_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
  const { shouldYouBuy } = config;
  ...
  return (
    <Section
      ...
      spacing="lg"
      className={cn('bg-[#FFF9F4]', className)}
      aria-labelledby={`${shouldYouBuy.id}-heading`}
    >
      <Container size="lg">
        <FadeUp className="mx-auto max-w-3xl space-y-3">
          <Heading
            as="h2"
            size="h2"
            id={`${shouldYouBuy.id}-heading`}
            className="tracking-tight"
          >
            {shouldYouBuy.title}
          </Heading>
          <MutedText className="text-base leading-relaxed sm:text-[1.05rem]">
            {shouldYouBuy.description}
          </MutedText>
        </FadeUp>
      </Container>
    </Section>
...
/** Views vs Likes vs Followers */
export function TikTokViewsServiceCompare({
  config = TIKTOK_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
    ...
    >
      <Container size="xl">
        ...
        <ul className="grid gap-4 md:grid-cols-3 md:gap-5">
          <li>
            <FadeUp>
              <article className="flex h-full flex-col rounded-2xl border border-[color-mix(in_srgb,var(--brand-primary)_22%,var(--border-subtle))] bg-white p-5 shadow-[0_14px_32px_-22px_rgba(28,25,23,0.28)] sm:p-6">
                <p className="text-[11px] font-bold tracking-[0.12em] text-[var(--brand-primary)] uppercase">
                  Current focus
                </p>
...
export function TikTokViewsBeforeBuying({
  config = TIKTOK_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
/** Popular package sizes — cards */
export function TikTokViewsPackageSizes({
  config = TIKTOK_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
/** Best practices */
export function TikTokViewsBestPractices({
  config = TIKTOK_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
export function TikTokViewsRelatedArticles({
  articles,
  title = 'Related Articles',
  description = 'Guides that help you grow on TikTok alongside view packages.',
}: {
