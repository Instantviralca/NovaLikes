...
import {
  YOUTUBE_VIEWS_PAGE_CONFIG,
  type YouTubeViewsPageConfig,
} from '@/data/content/youtube-views-page-config';
import { cn } from '@/lib/utils';
import type { BenefitItem, CtaProps, ProcessStep } from '@/types/components';
import type { InternalLink } from '@/types/linking';

/** Tighter authority rhythm matching YouTube Subscribers. */
const YT_SECTION_SPACING = 'py-7 md:py-9 lg:py-10';

type SectionProps = {
  config?: YouTubeViewsPageConfig;
  className?: string;
};
...
/** Section 4 — Why do people buy YouTube Views */
export function YouTubeViewsWhyBuy({
  id,
  title,
  description,
  items,
    ...
    >
      <Container size="xl">
        <div className="grid items-center gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:gap-12 lg:items-center">
          ...
          >
            <YouTubeViewsAnalyticsDashboard className="max-w-none" />
          </FadeUp>
        </div>
      </Container>
    </Section>
...
/** Section 5 — Can you buy YouTube Views? */
export function YouTubeViewsCanYouBuy({
  config = YOUTUBE_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
    ...
    >
      <Container size="xl">
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:gap-14">
          ...
          >
            <YouTubeViewsOrderStatusDashboard className="max-w-none" />
          </FadeUp>
...
/** Section 6 — How to buy YouTube Views */
export function YouTubeViewsHowToBuy({
  id,
  title,
  description,
  steps,
...
/** Section 8 — Does buying YouTube Views help? */
export function YouTubeViewsDoesBuyingHelp({
  config = YOUTUBE_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
      ...
      <Container size="xl">
        ...
        <div className="grid items-stretch gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.25fr)_minmax(0,0.9fr)] lg:gap-6 xl:gap-7">
          ...
          <FadeUp delay={0.04} className="flex items-center justify-center lg:px-0">
            <YouTubeViewsTrafficSourcesDashboard className="w-full max-w-[30rem]" />
          </FadeUp>
...
/** Section 9 — What happens after you buy YouTube Views */
export function YouTubeViewsWhatHappens({
  config = YOUTUBE_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
/** Section 10 — Views vs Subscribers vs Watch Time */
export function YouTubeViewsServiceCompare({
  config = YOUTUBE_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
          ...
          <li>
            <FadeUp delay={0.04}>
              <article className="flex h-full flex-col rounded-2xl border border-[var(--border-subtle)] bg-white p-5 shadow-[0_14px_32px_-22px_rgba(28,25,23,0.28)] transition-[transform,box-shadow,border-color] duration-300 hover:-translate-y-1 hover:border-[var(--brand-primary)] hover:shadow-[0_24px_48px_-20px_rgba(249,115,22,0.35)] motion-reduce:hover:translate-y-0 sm:p-6">
                ...
                <Link
                  href={serviceCompare.subscribers.href}
...
export function YouTubeViewsPackageSizes({
  config = YOUTUBE_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
export function YouTubeViewsBestPractices({
  config = YOUTUBE_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
    ...
    >
      <Container size="xl">
        ...
        <FadeUp delay={0.04} className="mx-auto mb-7 max-w-[28rem]">
          <YouTubeViewsAudienceRetentionDashboard className="max-w-none" />
        </FadeUp>
...
export function YouTubeViewsCommonMistakes({
  config = YOUTUBE_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
export function YouTubeViewsExploreMore({
  config = YOUTUBE_VIEWS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
export function YouTubeViewsRelatedArticles({
  articles,
  title = 'Related Articles',
  description = 'Guides that help you grow stronger YouTube video performance alongside view packages.',
}: {
