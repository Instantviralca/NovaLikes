...
import {
  YOUTUBE_SUBSCRIBERS_PAGE_CONFIG,
  type YouTubeSubscribersPageConfig,
} from '@/data/content/youtube-subscribers-page-config';
import { cn } from '@/lib/utils';
import type { BenefitItem, CtaProps, ProcessStep } from '@/types/components';
import type { InternalLink } from '@/types/linking';

/** Tighter authority rhythm matching FB Page Likes / Post Likes. */
const YT_SECTION_SPACING = 'py-7 md:py-9 lg:py-10';

type SectionProps = {
  config?: YouTubeSubscribersPageConfig;
  className?: string;
};
...
export function YouTubeSubscribersWhyChoose({
  ...
  return (
    <Section
      id={whyChoose.id}
      spacing="none"
      className={cn(YT_SECTION_SPACING, 'bg-white', className)}
...
/** Section 4 — Why do people buy YouTube Subscribers */
export function YouTubeSubscribersWhyBuy({
  id,
  title,
  description,
  items,
  ...
  return (
    <Section
      id={id}
      spacing="none"
      className={cn(YT_SECTION_SPACING, 'bg-[#FFF9F4]', className)}
    ...
    >
      <Container size="xl">
        <div className="grid items-center gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:gap-12 lg:items-center">
          ...
          >
            <YouTubeSubscribersOrderStatusDashboard className="max-w-none" />
          </FadeUp>
        </div>
      </Container>
    </Section>
...
/** Section 5 — Can you buy YouTube Subscribers? */
export function YouTubeSubscribersCanYouBuy({
  config = YOUTUBE_SUBSCRIBERS_PAGE_CONFIG,
  className,
}: SectionProps) {
    ...
    >
      <Container size="xl">
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:gap-14">
          ...
          >
            <YouTubeSubscribersChannelOverviewDashboard className="max-w-none" />
          </FadeUp>
...
/** Section 6 — How to buy YouTube Subscribers */
export function YouTubeSubscribersHowToBuy({
  id,
  title,
  description,
  steps,
...
/** Section 8 — Does buying YouTube Subscribers help? */
export function YouTubeSubscribersDoesBuyingHelp({
  config = YOUTUBE_SUBSCRIBERS_PAGE_CONFIG,
  className,
}: SectionProps) {
      ...
      <Container size="xl">
        ...
        <div className="grid items-stretch gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.25fr)_minmax(0,0.9fr)] lg:gap-6 xl:gap-7">
          ...
          <FadeUp delay={0.04} className="flex items-center justify-center lg:px-0">
            <YouTubeSubscribersAnalyticsDashboard className="w-full max-w-[30rem]" />
          </FadeUp>
...
/** Section 9 — What happens after you buy YouTube Subscribers */
export function YouTubeSubscribersWhatHappens({
  config = YOUTUBE_SUBSCRIBERS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
/** Section 10 — Subscribers vs Views vs Watch Time */
export function YouTubeSubscribersServiceCompare({
  config = YOUTUBE_SUBSCRIBERS_PAGE_CONFIG,
  className,
}: SectionProps) {
    ...
    >
      <Container size="xl">
        ...
        <ul className="grid gap-4 md:grid-cols-3 md:gap-5">
          <li>
            <FadeUp>
              <article className="flex h-full flex-col rounded-2xl border border-[color-mix(in_srgb,#FF0000_22%,var(--border-subtle))] bg-white p-5 shadow-[0_14px_32px_-22px_rgba(28,25,23,0.28)] transition-[transform,box-shadow,border-color] duration-300 hover:-translate-y-1 hover:border-[var(--brand-primary)] hover:shadow-[0_24px_48px_-20px_rgba(249,115,22,0.35)] motion-reduce:hover:translate-y-0 sm:p-6">
                <span className="mb-3 flex size-11 items-center justify-center rounded-xl bg-[#FFF1F2] text-[#FF0000]">
                ...
                <h3 className="mt-2 text-base font-semibold text-[var(--text-primary)]">
                  {serviceCompare.subscribers.title}
                </h3>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-[var(--text-secondary)]">
                  {serviceCompare.subscribers.description}
                </p>
                <span className="mt-4 inline-flex w-fit rounded-full bg-[linear-gradient(145deg,var(--brand-accent-soft)_0%,#ffd7b8_100%)] px-3.5 py-1.5 text-sm font-semibold text-[var(--brand-primary)]">
                  {serviceCompare.subscribers.ctaLabel}
                </span>
              </article>
            </FadeUp>
          </li>
          ...
          <li>
            <FadeUp delay={0.08}>
              <article className="flex h-full flex-col rounded-2xl border border-[var(--border-subtle)] bg-white p-5 shadow-[0_14px_32px_-22px_rgba(28,25,23,0.28)] transition-[transform,box-shadow,border-color] duration-300 hover:-translate-y-1 hover:border-[var(--brand-primary)] hover:shadow-[0_24px_48px_-20px_rgba(249,115,22,0.35)] motion-reduce:hover:translate-y-0 sm:p-6">
                <span className="mb-3 flex size-11 items-center justify-center rounded-xl bg-[#FFF7ED] text-[#EA580C]">
                  <Clock3 className="size-5" aria-hidden />
                </span>
                <h3 className="text-base font-semibold text-[var(--text-primary)]">
                  {serviceCompare.watchTime.title}
                </h3>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-[var(--text-secondary)]">
                  {serviceCompare.watchTime.description}
                </p>
                <Link
                  href={serviceCompare.watchTime.href}
                  className="mt-4 inline-flex text-sm font-semibold text-[var(--brand-primary)] underline-offset-2 hover:underline"
                >
                  {serviceCompare.watchTime.ctaLabel}
                </Link>
              </article>
            </FadeUp>
          </li>
  ...
  return (
    ...
    >
      <Container size="xl">
        ...
        <ul className="mt-8 grid gap-5 sm:grid-cols-2 sm:gap-6 lg:gap-7">
...
/** Section 12 — Popular package sizes */
export function YouTubeSubscribersPackageSizes({
  config = YOUTUBE_SUBSCRIBERS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
export function YouTubeSubscribersBestPractices({
  config = YOUTUBE_SUBSCRIBERS_PAGE_CONFIG,
  className,
}: SectionProps) {
    ...
    >
      <Container size="xl">
        ...
        <FadeUp delay={0.04} className="mx-auto mb-7 max-w-[28rem]">
          <YouTubeSubscribersAnalyticsDashboard className="max-w-none" />
        </FadeUp>
...
export function YouTubeSubscribersCommonMistakes({
  config = YOUTUBE_SUBSCRIBERS_PAGE_CONFIG,
  className,
}: SectionProps) {
        ...
        <ul className="grid gap-4 sm:grid-cols-2 lg:gap-5">
          {commonMistakes.items.map((item, index) => {
            ...
            return (
              <li key={item.id}>
                <FadeUp delay={0.03 * index}>
                  <article className="flex h-full gap-4 rounded-2xl border border-[var(--border-subtle)] bg-[#FFF9F4] p-5 shadow-[0_14px_32px_-22px_rgba(28,25,23,0.28)] sm:gap-5 sm:p-6">
                    <span className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-white text-stone-500 shadow-sm">
...
/** Section 16 — Explore more YouTube growth services / resources */
export function YouTubeSubscribersExploreMore({
  config = YOUTUBE_SUBSCRIBERS_PAGE_CONFIG,
  className,
}: SectionProps) {
  const { exploreMore } = config;
  ...
  return (
    <Section
      ...
      spacing="none"
      className={cn(YT_SECTION_SPACING, 'bg-[#FFF9F4]', className)}
      aria-labelledby={`${exploreMore.id}-heading`}
    >
      <Container size="xl">
        <FadeUp immediate className="mx-auto mb-7 max-w-3xl space-y-3 text-center">
          <Heading
        ...
        <ul
          className={cn(
            'mx-auto grid gap-5',
            exploreMore.cards.length === 1
              ? 'max-w-md'
              : exploreMore.cards.length === 2
                ? 'max-w-3xl sm:grid-cols-2'
                : 'md:grid-cols-3',
          )}
        >
          {exploreMore.cards.map((card, index) => (
            <li key={card.id}>
              <FadeUp delay={0.03 * index}>
                <article className="flex h-full flex-col rounded-2xl border border-[var(--border-subtle)] bg-white p-5 shadow-[0_14px_32px_-22px_rgba(28,25,23,0.28)] sm:p-6">
                  ...
                  <Link
                    href={card.href}
                    className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-[var(--brand-primary)] underline-offset-2 hover:underline"
                  >
                    {card.ctaLabel}
                    <ArrowRight className="size-3.5" aria-hidden />
                  </Link>
...
export function YouTubeSubscribersRelatedArticles({
  articles,
  title = 'Related Articles',
  description = 'Guides that help you grow a stronger YouTube channel alongside subscriber packages.',
}: {
  ...
  return (
    <Section
      id="related-articles"
      spacing="none"
      className={cn(YT_SECTION_SPACING, 'bg-white')}
                ...
                >
                  <div className="relative aspect-[16/9] min-h-[10rem] overflow-hidden bg-[var(--surface-muted)] sm:min-h-[11rem]">
                    {article.image ? (
                      ...
                      <div
                        className="flex h-full w-full items-end bg-[linear-gradient(145deg,#fee2e2,#ef444422,#F9731622)] p-3"
                        aria-hidden
                      />
                    )}
