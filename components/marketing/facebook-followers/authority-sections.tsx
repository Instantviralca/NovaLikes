...
import { FacebookFollowersOrderStatusDashboard } from '@/components/illustrations/facebook-followers-order-status-dashboard';
...
import {
  FACEBOOK_FOLLOWERS_PAGE_CONFIG,
  type FacebookFollowersPageConfig,
} from '@/data/content/facebook-followers-page-config';
...
type SectionProps = {
  config?: FacebookFollowersPageConfig;
  className?: string;
};

/** Section 3 — Why do people buy Facebook followers */
export function FacebookFollowersWhyBuy({
  id,
  title,
  description,
  items,
    ...
    >
      <Container size="xl">
        <div className="grid items-center gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:gap-12 lg:items-center">
          ...
          <FadeUp delay={0.06} className="mx-auto flex w-full max-w-[28rem] items-center justify-center self-center lg:max-w-none">
            <FacebookFollowersAnalyticsDashboard className="max-w-none" />
          </FadeUp>
        </div>
      </Container>
    </Section>
...
/** Section 4 — Can you buy Facebook followers? */
export function FacebookFollowersCanYouBuy({
    ...
    >
      <Container size="xl">
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:gap-14">
          <FadeUp delay={0.04} className="order-2 mx-auto w-full max-w-[26rem] lg:order-1 lg:max-w-none">
            <FacebookFollowersOrderSummaryDashboard className="max-w-none" />
          </FadeUp>
...
/** Section 5 — How to buy Facebook followers (desktop vertical timeline) */
export function FacebookFollowersHowToBuy({
  id,
  title,
  description,
  steps,
...
/** Section 7 — Does buying Facebook followers help? */
export function FacebookFollowersDoesBuyingHelp({
  config = FACEBOOK_FOLLOWERS_PAGE_CONFIG,
  className,
}: SectionProps) {
      ...
      <Container size="xl">
        ...
        <div className="grid items-stretch gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.25fr)_minmax(0,0.9fr)] lg:gap-6 xl:gap-7">
          ...
          <FadeUp delay={0.04} className="flex items-center justify-center lg:px-0">
            <FacebookFollowersAnalyticsDashboard className="w-full max-w-[30rem]" />
          </FadeUp>
...
/** Section 8 — What happens when you buy Facebook followers */
export function FacebookFollowersWhatHappens({
  config = FACEBOOK_FOLLOWERS_PAGE_CONFIG,
  className,
}: SectionProps) {
  ...
  return (
    ...
    >
      <Container size="xl">
        ...
        <FadeUp delay={0.04} className="mx-auto mb-7 max-w-[26rem]">
          <FacebookFollowersOrderStatusDashboard className="max-w-none" />
        </FadeUp>
              ...
              <li key={step.id}>
                <FadeUp delay={0.03 * index}>
                  <article className="relative flex h-full flex-col rounded-2xl border border-[var(--border-subtle)] bg-white p-5 shadow-[0_14px_32px_-22px_rgba(28,25,23,0.28)] sm:p-6">
                    <span className="mb-3 flex size-9 items-center justify-center rounded-full bg-[linear-gradient(145deg,var(--brand-accent-soft)_0%,#ffd7b8_100%)] text-sm font-bold text-[var(--brand-primary)]">
                      {index + 1}
                    </span>
                    ...
                    {isLast ? (
                      <Link
                        ...
                        className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-[var(--brand-primary)] underline-offset-2 hover:underline"
                      >
                        <Check className="size-3.5" strokeWidth={2.75} aria-hidden />
                        Continue Growing Your Community
                      </Link>
                    ) : null}
                  </article>
                </FadeUp>
...
/** Section 9 — Followers vs Page Likes vs Post Likes */
export function FacebookFollowersServiceCompare({
  config = FACEBOOK_FOLLOWERS_PAGE_CONFIG,
  className,
}: SectionProps) {
          ...
          <li>
            <FadeUp delay={0.04}>
              <article className="flex h-full flex-col rounded-2xl border border-[var(--border-subtle)] bg-white p-5 shadow-[0_14px_32px_-22px_rgba(28,25,23,0.28)] transition-[transform,box-shadow,border-color] duration-300 hover:-translate-y-1 hover:border-[var(--brand-primary)] hover:shadow-[0_24px_48px_-20px_rgba(249,115,22,0.35)] motion-reduce:hover:translate-y-0 sm:p-6">
                ...
                <Link
                  href={serviceCompare.pageLikes.href}
...
/** Section 10 — What to check before buying */
export function FacebookFollowersBeforeBuying({
  config = FACEBOOK_FOLLOWERS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
export function FacebookFollowersPackageSizes({
  config = FACEBOOK_FOLLOWERS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
export function FacebookFollowersBestPractices({
  config = FACEBOOK_FOLLOWERS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
export function FacebookFollowersCommonMistakes({
  config = FACEBOOK_FOLLOWERS_PAGE_CONFIG,
  className,
}: SectionProps) {
...
/** Section 17 — Related articles with premium cards */
export function FacebookFollowersRelatedArticles({
  articles,
  title = 'Related Articles',
  description = 'Guides that help you grow on Facebook alongside follower packages.',
}: {
